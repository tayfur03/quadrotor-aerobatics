%% DEMO_AIT_STAR_DIRECT
% Direct AIT*-style radar-aware planning demo.

clear; clc; close all;
clear ait_star_planner_direct build_direct_planner_context;
rehash;
...
fprintf('=== Direct AIT* Demo ===\n');
fprintf('Using ait_star_planner_direct: %s\n', which('ait_star_planner_direct'));
...
cfg.rng_seed = 7;
cfg.radar_position_mode = 'figure_click'; % 'manual_ne' | 'manual_fraction' | 'figure_click'
cfg.num_radars = 1;
cfg.radar_manual_ne = [];
...
planner_params.batch_size = 180;
planner_params.max_nodes = 12000;
...
planner_params.profile_ait = true;
planner_params.neighbor_precompute_mode = 'batch';
planner_params.use_parallel_reverse = true;
planner_params.use_parallel_edge_checks = true;
planner_params.parallel_threshold_neighbors = 64;
planner_params.parallel_threshold_edges = 32;
planner_params.reverse_batch_repair_limit = 256;
planner_params.forward_expand_chunk = 64;
planner_params.kd_rebuild_interval = 1;
planner_params.graph_rebuild_interval = 3;
planner_params.reverse_full_rebuild_interval = 3;
planner_params.reverse_debug = true;

t_total = tic;
[path_world, final_cost, info] = ait_star_planner_direct(start_world, goal_world, env, planner_params);
...
fatrop_ok = false;
fatrop_info = struct('message', '', 'solve_time_s', NaN, 'solver', 'fatrop');
path_fatrop_ned = [];
fatrop_cost = NaN;
if cfg.fatrop_enable && ~isempty(path_ned) && size(path_ned, 2) >= 2
    try
        ...
        fatrop_ok = true;
        fprintf('FATROP trajectory: %d samples, solve time %.2f s\n', ...
            size(path_fatrop_ned, 2), fatrop_info.solve_time_s);
    catch ME
...
fig_path = figure(1);
clf(fig_path);
set(fig_path, 'Name', 'Direct AIT* Path on Terrain', 'Position', [100, 80, 1180, 820], ...
...
title('Terrain Mesh With Direct AIT* Path and Radar Coverage');
view(38, 30);
axis tight;
grid on;
...
fig_diag = figure(2);
clf(fig_diag);
set(fig_diag, 'Name', 'Direct AIT* Diagnostics', 'Position', [130, 100, 1240, 520], ...
    'Color', 'w', 'Visible', 'on', 'WindowStyle', 'normal');
...
title('Top-Down Threat Map and Final Path');
xlabel('North [m]');
ylabel('East [m]');
colormap(gca, turbo);
colorbar;
...
subplot(1, 2, 2);
scatter(info.sampled_nodes(1, :), info.sampled_nodes(2, :), 12, [0.72, 0.72, 0.72], 'filled');
hold on;
tree_idx = find(info.in_tree_fwd);
for i = 1:numel(tree_idx)
    idx = tree_idx(i);
    p = double(info.parent_fwd(idx));
    if p > 0
        plot(info.sampled_nodes(1, [p, idx]), info.sampled_nodes(2, [p, idx]), ...
            '-', 'Color', [0.15, 0.42, 0.78], 'LineWidth', 0.9);
    end
end
if ~isempty(path_world)
    plot(path_world(1, :), path_world(2, :), '-', 'Color', [0.92, 0.15, 0.18], 'LineWidth', 2.2);
...
axis equal tight;
grid on;
title('Accepted Samples, Forward Tree, and Final Path');
xlabel('North [m]');
ylabel('East [m]');
...
fprintf('\nThreat map:            %.2f s\n', t_threat_s);
fprintf('Reverse heuristic:     %.2f s\n', info.reverse_time_s);
fprintf('  Neighbor precompute: %.2f s\n', info.neighbor_precompute_time_s);
fprintf('  Reverse rebuild:     %.2f s\n', info.reverse_rebuild_time_s);
fprintf('  Reverse repair:      %.2f s\n', info.reverse_repair_time_s);
fprintf('Forward search:        %.2f s\n', info.forward_time_s);
fprintf('  Queue rebuilds:      %.2f s\n', info.forward_queue_build_time_s);
fprintf('  Goal probes:         %.2f s\n', info.goal_probe_time_s);
fprintf('  Edge validation:     %.2f s\n', info.edge_validation_time_s);
fprintf('Total planner:         %.2f s\n', t_total_s);
fprintf('AIT* iterations:       %d\n', info.iterations);
...
fprintf('Invalid edges:         %d\n', info.invalid_edge_count);
fprintf('Reverse repairs:       %d\n', info.reverse_repairs);
fprintf('Reverse queue pops:    %d\n', info.reverse_queue_pops);
fprintf('Forward edge checks:   %d\n', info.forward_edge_checks);
fprintf('Valid edge cache hits: %d\n', info.valid_edge_cache_hits);
fprintf('Forward stale pops:    %d\n', info.forward_stale_pops);
fprintf('Goal probe checks:     %d\n', info.goal_probe_checks);
fprintf('Goal probe successes:  %d\n', info.goal_probe_successes);
fprintf('Heuristic-ready nodes: %d\n', info.adaptive_heuristic_ready_count);
fprintf('Parallel reverse:      %d\n', info.parallel_used_reverse);
fprintf('Parallel edge checks:  %d\n', info.parallel_used_edge_checks);
fprintf('RNG seed:              %d\n', cfg.rng_seed);
...
info = struct('solver', 'fatrop', 'solve_time_s', NaN, 'message', '');
