...
%   alpha_ref : Açısal ivme referansı (Body frame) [3x1] (Feedforward)
    ...
    r_psi_sq = bx(1)^2 + bx(2)^2; % r_w^T * r_w
    %r_psi = [bx(1) bx(2) 0]'; % r_psi vector


    
    if r_psi_sq < 1e-4
        % Singularity koruması (Pitch 90 derece durumu)
        S = [0, 0, 1]; 
    else
        % S = [-bx(2), bx(1)] * [0 -bz(1) by(1); 0 -bz(2) by(2)] / r_sq
        term1 = [-bx(2), bx(1)] / r_psi_sq;
        mat2  = [0, -bz(1), by(1);
                 0, -bz(2), by(2)];
        S = term1 * mat2;
    end
    ...
    % Matrisin sol tarafı: [tau * R * [iz]x^T , bz; S, 0]
    
    iz_skew = [0 -1 0; 1 0 0; 0 0 0]; % [iz]x
    iz_skew_T = iz_skew'; 
    
    % M matrisi (4x4)
    top_left  = tau * R * iz_skew_T; % 3x3
    top_right = bz;                  % 3x1
    bot_left  = S;                   % 1x3
    bot_right = 0;                   % 1x1
    ...
    % Terim: R * (2*tau_dot + tau*[Omega]x) * [iz]x^T * Omega
    term_coriolis_1 = R * (2*tau_dot_ref*eye(3) + tau*Omega_skew) * iz_skew_T * Omega_ref;
    
    % Terim: S_dot * Omega (S matrisinin türevi karmaşıktır, ihmal edilebilir 
    % veya sayısal farkla bulunabilir. Makalede analitik formül verilmemiş, 
    % ancak S*Omega = psi_dot olduğu için S_dot*Omega terimi yerine 
    % yaklaşık olarak psi_ddot hesaplarında kullanılır. 
    % Tam doğruluk için S'in analitik türevi gerekir. 
    % Burada basitleştirme adına S_dot ~ 0 varsayıyoruz veya numerik eklenebilir.
    term_coriolis_2 = 0; 
    ...
    omega_dot_ref = sol_15(1:3); % Bu, feedforward Snap terimidir.
