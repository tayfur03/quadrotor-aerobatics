function terrain = terrain_generator(type, params)
...
% Author: Quadrotor Terrain Following Project
    ...
    switch lower(type)
        ...
        case 'ridge'
            ...
            ridge_width = wavelength / 2;
            dist_from_ridge = abs(E_grid - ridge_center_E);
            Z = base_height + amplitude * exp(-(dist_from_ridge.^2) / (2*ridge_width^2));
            % Add secondary features
            Z = Z + amplitude * 0.2 * sin(2*pi*N_grid/wavelength);
        ...
        case 'mountain'
            ...
            Z = base_height + amplitude * exp(-(dist_from_peak.^2) / (2*wavelength^2));
            % Add ridges radiating from peak
            angle = atan2(E_grid - peak_E, N_grid - peak_N);
            Z = Z + amplitude * 0.2 * sin(4*angle) .* exp(-dist_from_peak / wavelength);
